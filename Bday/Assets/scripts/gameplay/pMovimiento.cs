...
public class pMovimiento : MonoBehaviour
{
    ...
    public void progresoMinijuego()
    {
        cuartonum = int.Parse(ultimoCuarto)-1;

        entradas[cuartonum].SetActive(false);
        salidas[cuartonum].SetActive(true);
        generadores[cuartonum].GetComponent<Animator>().SetBool("destruido", true);
        generadores[cuartonum].GetComponent<BoxCollider2D>().isTrigger = false;
    }
